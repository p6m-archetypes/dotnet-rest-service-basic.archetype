...
public class {{ PrefixName }}{{ SuffixName }}Client : I{{ PrefixName }}{{ SuffixName }}Service
{
    ...
    {
        var url = $"{_baseUrl}/api/{{ PrefixName }}{{ SuffixName }}";
        Console.WriteLine($"[{{ PrefixName }}{{ SuffixName }}Client] POST {url}");
        Console.WriteLine($"[{{ PrefixName }}{{ SuffixName }}Client] Authorization header present: {_httpClient.DefaultRequestHeaders.Authorization != null}");
