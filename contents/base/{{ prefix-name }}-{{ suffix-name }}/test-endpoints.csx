...
Console.WriteLine("Testing endpoints on http://localhost:8081:");
Console.WriteLine();

// Test root
var rootResponse = await client.GetAsync("http://localhost:8081/");
...
// Test health
var healthResponse = await client.GetAsync("http://localhost:8081/health");
Console.WriteLine($"GET /health: {healthResponse.StatusCode}");

// Test swagger
var swaggerResponse = await client.GetAsync("http://localhost:8081/swagger");
Console.WriteLine($"GET /swagger: {swaggerResponse.StatusCode}");

// Test auth endpoint with POST
var authResponse = await client.PostAsync("http://localhost:8081/api/auth/token", 
...
var apiResponse = await client.GetAsync("http://localhost:8081/api/{{ PrefixName }}{{ SuffixName }}");
